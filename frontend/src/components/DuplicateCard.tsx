...
export default function DuplicateCard({ set, onToggleKeep, onDelete }: DuplicateCardProps) {
    ...
    <div className="rounded-lg border bg-card">
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Badge variant={statusVariant(set.status)}>{set.status ?? "unknown"}</Badge>
          {set.scan_method && (
            <Badge variant="outline">{set.scan_method}</Badge>
          )}
      ...
      {expanded && (
        <div className="border-t px-4 py-4">
          <FileComparison
            files={set.files}
            onToggleKeep={
              onToggleKeep ? (fileId, keep) => onToggleKeep(set.id, fileId, keep) : undefined
            }
          />
