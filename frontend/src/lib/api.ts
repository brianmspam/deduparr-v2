...
// Safely parse JSON; fall back to { detail: text } for non‑JSON responses
...
export const scanAPI = {
    ...
    getDuplicates: async (params?: {
        ...
        if (params?.search) qs.set("search", params.search);  // ← ADD
        if (params?.limit) qs.set("limit", String(params.limit));
        if (params?.offset) qs.set("offset", String(params.offset));
        const queryString = qs.toString();
        return fetchAPI<DuplicatesResponse>(`/api/scan/duplicates${queryString ? `?${queryString}` : ""}`);
    },
    ...
    deleteSet: (setId: number, dryRun: boolean) =>
        postAPI<Record<string, unknown>>(`/api/scan/duplicates/${setId}/delete`, {
            dry_run: dryRun,
        }),
    updateFileKeep: (setId: number, fileId: number, keep: boolean) =>
        patchAPI<{ id: number; keep: boolean }>(
...
export const setupAPI = {
    ...
    testPlex: () =>
        postAPI<{ success: boolean; message: string }>("/api/setup/plex/test"),
    getServers: (token: string) =>
        postAPI<Array<{ name: string; address: string }>>(
            "/api/setup/plex/servers",
            { token },
        ),
};
...
export const scoringAPI = {
    ...
    deleteRule: (id: number) =>
        deleteAPI<{ status: string }>(`/api/scoring/rules/${id}`),

    // Folder priority endpoints using safeJson + existing error extraction
    scanFolderPriority: async (minCount: number) => {
        const res = await fetch(`${BASE_URL}/api/scoring/folder-priority/scan?min_count=${minCount}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        });
        const text = await res.text().catch(() => "");
        ...
        return body as { folders: { path: string; file_count?: number }[] } | null;
    },

    listFolderPriority: async (): Promise<FolderPriorityRow[]> => {
        const res = await fetch(`${BASE_URL}/api/scoring/folder-priority`, {
            method: "GET",
            headers: { "Content-Type": "application/json" },
        });
        const text = await res.text().catch(() => "");
        ...
        return (body || []) as FolderPriorityRow[];
    },
    ...
    updateFolderPriority: async (
        ...
        const res = await fetch(`${BASE_URL}/api/scoring/folder-priority/${id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
